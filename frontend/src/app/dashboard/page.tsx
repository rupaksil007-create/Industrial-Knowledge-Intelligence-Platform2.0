...
export default function Dashboard() {
  ...
  // Ingestion status: IDLE | INGESTING | READY
  const [ingestionStatus, setIngestionStatus] = useState<"IDLE" | "INGESTING" | "READY">("IDLE");
  ...
  const fetchIngestionStatus = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setIngestionStatus(data.status as "IDLE" | "INGESTING" | "READY");
  ...
  // Poll ingestion status until READY or IDLE, then stop
  const startIngestionPoll = () => {
   ...
   * Posts to /compliance/analyze which now BLOCKS until the full analysis
   * completes on the backend. The response IS the completed report.
   * No polling. No multiple clicks needed.
   */
  const handleAnalyzeCompliance = async () => {
    // Guard: do not run if ingestion is in progress
    if (ingestionStatus === "INGESTING") {
      setComplianceError("Document ingestion is in progress. Please wait until it completes before running an audit.");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        // Backend returns the COMPLETED report directly — no polling needed
        const data = await res.json();
        setComplianceData(data);
        setComplianceError("");
      } else if (res.status === 409) {
        const err = await res.json().catch(() => ({ detail: "Ingestion in progress." }));
        setComplianceError(err.detail || "Cannot audit while ingestion is running. Please wait.");
      } else {
        setComplianceError("Compliance analysis failed. Check backend logs.");
      }
    } catch (e: any) {
      console.error("Error analyzing compliance:", e);
      setComplianceError("Network error: Could not reach compliance engine.");
    } finally {
  ...
  // Cleanup ingestion poll on unmount
  useEffect(() => {
    return () => {
      if (ingestionPollRef.current) clearInterval(ingestionPollRef.current);
    };
  ...
  const handleUpload = async (file: File) => {
    ...
    setIsUploading(true);
    setIngestionStatus("INGESTING");  // immediately mark as ingesting
    ...
    try {
      ...
      if (res.ok) {
        ...
        fetchDocuments();
        // Start polling ingestion status until backend says READY
        startIngestionPoll();
        setTimeout(() => {
  ...
  const handleDeleteDocument = async (id: string) => {
    ...
    setIngestionStatus("INGESTING");  // mark as ingesting during delete
    try {
      ...
      if (res.ok) {
        fetchDocuments();
        // Poll until backend marks ingestion complete
        startIngestionPoll();
      } else {
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-zinc-500 mt-1">Industrial safety & operational audit engine powered by RAG gap analysis</p>
              </div>
              <div className="flex items-center gap-3">
                {ingestionStatus === "INGESTING" && (
                  <span className="text-[10px] font-mono text-amber-400 animate-pulse uppercase tracking-wider">
                ...
                <button
                  ...
                  disabled={isAnalyzingCompliance || ingestionStatus === "INGESTING"}
                  className="flex items-center space-x-2 px-4 py-2.5 rounded bg-cyan-500 hover:bg-cyan-400 text-zinc-950 text-xs font-bold font-mono uppercase tracking-wider disabled:opacity-50 disabled:cursor-not-allowed transition"
                >
            ...
            {/* Error Banner */}
            {complianceError && (
              <div className="flex items-center space-x-3 bg-rose-950/30 border border-rose-500/40 text-rose-400 p-4 rounded text-sm font-mono">
                <AlertTriangle className="h-5 w-5 shrink-0" />
            ...
            {/* Loading state */}
            {isAnalyzingCompliance && (
              <div className="glass-panel p-10 rounded-lg border border-zinc-800 flex flex-col items-center justify-center space-y-4">
                <div className="h-10 w-10 border-2 border-cyan-500 border-t-transparent rounded-full animate-spin" />
                <p className="text-xs font-mono text-cyan-400 uppercase animate-pulse">Running full compliance gap analysis... This may take up to 60 seconds.</p>
              </div>
            )}

            {/* No data yet */}
            {!isAnalyzingCompliance && !complianceData && (
              <div className="glass-panel p-10 rounded-lg border border-zinc-800 flex flex-col items-center justify-center space-y-4 text-center">
                <Shield className="h-12 w-12 text-zinc-700" />
                <div>
                  <p className="text-zinc-450 font-mono text-sm">No compliance report available.</p>
                  <p className="text-zinc-650 font-mono text-xs mt-1">Upload documents then click "Run Audit" to generate a report.</p>
                </div>
              </div>
            )}
              ...
              <>
                    ...
                    <div className="space-y-1.5 flex-1 min-w-0">
                      ...
                      <h3 className={`text-xl font-extrabold tracking-tight uppercase ${
                        complianceData.risk_level === "UNVERIFIED"
                          ? "text-zinc-500"
                          : complianceData.score >= 90
                          ? "text-emerald-400"
                          : complianceData.score >= 75
                          ? "text-cyan-400"
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  {[
                    { 
                      ...
                      value: complianceData.gaps_summary?.missing_procedures ?? 0,
                      desc: "Unresolved SOPs / guides"
                    },
                    { 
                      title: "Missing Inspections", 
                      value: complianceData.gaps_summary?.missing_inspections ?? 0,
                      desc: "Audit checksheets missing"
                    },
                    { 
                      title: "Missing Certifications", 
                      value: complianceData.gaps_summary?.missing_certifications ?? 0,
                      desc: "Expired or missing licenses"
                    },
                    { 
                      title: "Missing Safety Records", 
                      value: complianceData.gaps_summary?.missing_safety_records ?? 0,
                  ...
                  <div className="glass-panel p-5 rounded-lg border border-zinc-800 bg-[#09090b]/40">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {Object.entries(complianceData.coverage_matrix).map(([cat, val]: any) => {
                        ...
                        return (
                          <div key={cat} className="flex flex-col justify-between bg-zinc-950/60 border border-zinc-800 p-4 rounded hover:border-zinc-700 transition-all gap-3">
                            ...
                            <div className="w-full bg-zinc-800 rounded-full h-1.5 overflow-hidden">
                              <div 
                                className={`h-full rounded-full transition-all duration-500 ${
                                  level === "Fully Covered" ? "bg-emerald-500" :
                                  level === "Partially Covered" ? "bg-amber-500" : "bg-rose-500"
                                }`}
                                style={{ width: `${val}%` }}
                              />
                            </div>
                            ...
                            {details.documents_matched && details.documents_matched.length > 0 && (
                              <div className="flex flex-wrap gap-1 text-[8px] font-mono text-zinc-600 border-t border-zinc-900/60 pt-2 mt-1">
                                ...
                                {details.documents_matched.map((doc: string, dIdx: number) => (
                                  <span key={dIdx} className="bg-zinc-900 border border-zinc-800 px-1.5 py-0.5 rounded truncate max-w-[90px]" title={doc}>
                                    {doc}
                                  </span>
                                ))}
                              </div>
                ...
                <div className="glass-panel rounded-lg border border-zinc-800 overflow-hidden flex flex-col">
                  <div className="px-4 py-3 bg-[#121214] border-b border-zinc-800 flex justify-between items-center">
                    ...
                    <span className="text-[10px] text-zinc-550 font-mono">Showing {complianceData.gaps?.length ?? 0} gaps</span>
                  </div>
                  ...
                  {!complianceData.gaps || complianceData.gaps.length === 0 ? (
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full border-collapse text-left font-sans text-xs">
                        ...
                        <tbody className="divide-y divide-zinc-800 bg-[#09090b]/10">
                          {complianceData.gaps.map((gap: any, gIdx: number) => (
                            <tr key={gIdx} className="hover:bg-zinc-900/30 group transition-colors">
                                  ...
                                  {gap.nodes_involved && gap.nodes_involved.length > 0 && (
                                    <div className="flex flex-wrap gap-1 mt-1.5 items-center">
                                      ...
                                      {gap.nodes_involved.map((n: string, ni: number) => (
                                        <span key={ni} className="bg-zinc-900 border border-zinc-800 text-zinc-400 text-[9px] px-1.5 py-0.5 rounded font-mono">
                                          {n}
                                        </span>
                                      ))}
                                    </div>
                              ...
                              <td className="py-3 px-4">
                                <div className="max-w-xs text-[10px] text-zinc-500 bg-zinc-950/50 border border-zinc-800 p-2 rounded leading-relaxed max-h-[85px] overflow-y-auto font-mono whitespace-pre-wrap">
                                  {gap.evidence}
                                </div>
                              </td>
                            </tr>
